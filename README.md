...
- 饱和页面递归切分时间窗口；仍无法完整读取时创建可见的数据缺口。
...
- 5 分钟、小时、每日三层趋势保留；D1 达到 400 MB 安全水位时提前归档。
- Workers Free 额度预算：GraphQL 240 次/5 分钟软上限、Queue 20% 安全余量、D1 每日写入 80% 停线保护。
- Cloudflare Access JWT 验证、状态修改同源保护、严格安全响应头。
- SMTP 465 隐式 TLS / 587 STARTTLS 告警，密码以 AES-GCM 加密保存。
