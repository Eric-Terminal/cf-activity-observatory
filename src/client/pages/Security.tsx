...
export function Security() {
  ...
  const events = useQuery({ queryKey: ["security-events", range], queryFn: () => endpoints.events(eventParams) });
  return <>
    ...
    <div className="dashboard-grid"><article className="panel"><header className="panel-title"><h2>{t("correctedSecurityTrend")}</h2><Badge tone="blue">{t("adjustedTrend")}</Badge></header>{metrics.isLoading ? <Loading /> : metrics.error ? <ErrorState error={metrics.error} /> : <Chart option={securityTrend(metrics.data)} />}</article><article className="panel"><header className="panel-title"><h2>{t("rule")}</h2><Badge tone="blue">{t("adjustedTrend")}</Badge></header>{rules.isLoading ? <Loading /> : rules.error ? <ErrorState error={rules.error} /> : <Chart option={ruleRanking(rules.data)} />}</article></div>
...
function securityTrend(data: Awaited<ReturnType<typeof endpoints.metrics>> | undefined): Record<string, unknown> {
  return { tooltip: { trigger: "axis" }, legend: { bottom: 0 }, grid: { left: 54, right: 18, top: 18, bottom: 48 }, xAxis: { type: "time" }, yAxis: { type: "value", splitLine: { lineStyle: { opacity: 0.18 } } }, series: data?.series.map((series) => ({ name: series.name, type: "line", stack: "security", areaStyle: { opacity: 0.12 }, showSymbol: false, data: series.points.map((point) => [point.bucket_start, Number(point.estimated_count)]) })) ?? [] };
}
