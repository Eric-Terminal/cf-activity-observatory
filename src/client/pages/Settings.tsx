...
function HealthPanel({ health }: { health: ReturnType<typeof useQuery<Awaited<ReturnType<typeof endpoints.health>>>> }) {
  ...
  return <section className="settings-section"><header><div><h2>{t("usage")}</h2><p>UTC {new Date().toISOString().slice(0, 10)}</p></div>{data && <Badge tone={data.status === "healthy" ? "good" : "warn"}>{t(data.status)}</Badge>}</header>{health.isLoading ? <Loading /> : health.error ? <ErrorState error={health.error} /> : data && <><div className="usage-grid"><Usage label={t("graphqlQueries")} value={data.usageToday.graphqlQueries.toLocaleString()} /><Usage label={t("d1Reads")} value={data.usageToday.d1RowsRead.toLocaleString()} /><Usage label={t("d1Writes")} value={data.usageToday.d1RowsWritten.toLocaleString()} /><Usage label={t("d1Storage")} value={`${formatBytes(data.usageToday.d1SizeAfter)} / ${formatBytes(data.d1WarningBytes)}`} /></div><div className="table-scroll"><table><thead><tr><th>{t("zone")}</th><th>{t("dataset")}</th><th>{t("lag")}</th><th>{t("lastSuccess")}</th><th>{t("failures")}</th></tr></thead><tbody>{data.cursors.map((cursor) => <tr key={`${cursor.zoneId}:${cursor.dataset}`}><td>{cursor.zoneName}</td><td><code>{cursor.dataset}</code></td><td>{formatTime(cursor.cursorAt)}</td><td>{formatTime(cursor.lastSuccessAt)}</td><td><Badge tone={cursor.consecutiveFailures ? "warn" : "good"}>{cursor.consecutiveFailures}</Badge></td></tr>)}</tbody></table></div>{data.dlqJobs > 0 && <Notice tone="warning"><strong>{data.dlqJobs} {t("dlqJobs")}</strong></Notice>}{data.gaps.length > 0 && <Notice tone="warning"><strong>{data.gaps.length} {t("gaps")}</strong><span>{data.gaps[0]?.reason}</span></Notice>}</>}</section>;
}
...
function SmtpPanel() {
  ...
  return <SmtpForm key={config.data?.updatedAt ?? 0} initial={smtpInitial(config.data)} refresh={() => void config.refetch()} title={t("smtp")} help={t("smtpHelp")} />;
}

function SmtpForm({ initial, refresh, title, help }: { initial: SmtpState; refresh: () => void; title: string; help: string }) {
  const { t } = useI18n();
  ...
  return <section className="settings-section"><header><div><h2>{title}</h2><p>{help}</p></div><label className="switch"><input type="checkbox" checked={form.enabled} onChange={(event) => update("enabled", event.target.checked)} /><span /></label></header><div className="form-grid"><label><span>{t("server")}</span><input value={form.host} onChange={(event) => update("host", event.target.value)} /></label><label><span>{t("port")}</span><select value={form.port} onChange={(event) => { const port = Number(event.target.value) as 465 | 587; update("port", port); update("tlsMode", port === 465 ? "implicit" : "starttls"); }}><option value="465">465 · TLS</option><option value="587">587 · STARTTLS</option></select></label><label><span>{t("username")}</span><input value={form.username} onChange={(event) => update("username", event.target.value)} /></label><label><span>{t("password")}</span><input type="password" value={form.password} placeholder={t("passwordHint")} onChange={(event) => update("password", event.target.value)} /></label><label><span>{t("sender")}</span><input type="email" value={form.senderAddress} onChange={(event) => update("senderAddress", event.target.value)} /></label><label><span>{t("recipients")}</span><input value={form.recipients} onChange={(event) => update("recipients", event.target.value)} /></label></div>{(save.error || test.error) && <p className="form-error">{(save.error ?? test.error)?.message}</p>}<div className="button-row"><button onClick={() => save.mutate()} disabled={save.isPending}>{save.isSuccess ? t("saved") : t("save")}</button><button className="primary" onClick={() => test.mutate()} disabled={test.isPending}>{test.isSuccess ? t("saved") : t("testEmail")}</button></div></section>;
}
