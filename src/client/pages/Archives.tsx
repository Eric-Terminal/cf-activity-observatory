...
export function Archives() {
  ...
  const exportUrl = (type: "requests" | "security", format: "json" | "ndjson") => `/api/v1/export?${new URLSearchParams({ type, format, limit: "500", ...dates })}`;
  return <>
    <PageHeader title={t("archives")} eyebrow="R2 · NDJSON.gz" />
    <Notice>{t("archiveNote")}</Notice>
    <section className="panel export-panel"><div><h2>{t("exportRecent")}</h2><p>{t("last24h")} · 500 {t("records")}</p></div><div className="button-row"><a className="button" href={requestExport}>{t("exportRequests")}</a><a className="button" href={securityExport}>{t("exportSecurity")}</a><a className="button" href={exportUrl("requests", "json")}>{t("requests")} · {t("exportJson")}</a><a className="button" href={exportUrl("requests", "ndjson")}>{t("requests")} · {t("exportNdjson")}</a><a className="button" href={exportUrl("security", "json")}>{t("security")} · {t("exportJson")}</a><a className="button" href={exportUrl("security", "ndjson")}>{t("security")} · {t("exportNdjson")}</a></div></section>
